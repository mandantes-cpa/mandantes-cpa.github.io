...
| `assets/images/work-samples/` | The location for the two local JPEG work-sample thumbnails. |
...
└── assets/
    ...
    └── work-samples/
        ├── quickbooks-online-thumbnail.jpg
        └── xero-thumbnail.jpg
    └── data/
        └── testimonials.csv
```
...
## Add Canva work-sample links and JPEG thumbnails safely

The work-sample cards use a **local JPEG thumbnail** and a **public Canva presentation view link**. They do not embed, open, or download a PDF.

| Work sample | Required local JPEG path | Canva URL placeholder in `index.html` |
| --- | --- | --- |
| QuickBooks Online | `assets/images/work-samples/quickbooks-online-thumbnail.jpg` | `REPLACE_WITH_QUICKBOOKS_PUBLIC_VIEW_LINK` |
| Xero | `assets/images/work-samples/xero-thumbnail.jpg` | `REPLACE_WITH_XERO_PUBLIC_VIEW_LINK` |

Export or prepare each thumbnail as a **1600 × 900 pixel**, **16:9 landscape**, **sRGB JPEG** at around **80–85 quality**. Keep it under **500 KB** where possible. This size is sharp for the responsive card layout while remaining lightweight for a static site. A 1920 × 1080 JPEG is also acceptable if you already have it, but 1600 × 900 is the recommended upload size.

Before publishing, review every visible thumbnail and presentation page. Redact or remove client names, account numbers, bank information, email addresses, tax identification data, passwords, and any other confidential material. Also review the file name, image metadata, Canva presentation title, slide notes, linked files, and sharing permissions. You are responsible for confirming that the public Canva view link and thumbnail are safe to publish.

To activate each card, upload the thumbnail at the exact local path above. Then open `index.html`, search for its `REPLACE_WITH_...` placeholder, and replace only that placeholder text with the corresponding Canva design identifier and any remaining required URL path. The easier alternative is to replace the entire `href` value with the complete public Canva view URL you copied from Canva. After saving, the full thumbnail card opens the presentation in a new tab. Until you provide a real link, the site clearly displays “Canva link awaiting URL” and prevents the placeholder from acting as a dead link.
...
| Work-sample thumbnails | Add the two JPEG files in `assets/images/work-samples/` with the exact names shown above. |
| Canva work-sample links | Search `index.html` for `REPLACE_WITH_` and replace each placeholder with its full public Canva view URL. |
...
| A thumbnail says it is awaiting a JPEG | Add the image at the exact path in the table above, commit it, then wait for the GitHub Pages deployment to finish. |
| The thumbnail appears but does not open Canva | Confirm that you replaced the entire `href` value with a current public Canva **view** URL, and confirm in a private browser window that the Canva sharing setting allows anyone with the link to view it. |
...
The required editable placeholders are the two thumbnail JPEGs (`quickbooks-online-thumbnail.jpg` and `xero-thumbnail.jpg`), their two public Canva view URLs (`REPLACE_WITH_QUICKBOOKS_PUBLIC_VIEW_LINK` and `REPLACE_WITH_XERO_PUBLIC_VIEW_LINK`), the canonical URL placeholders (`YOUR_GITHUB_USERNAME` and `YOUR_REPOSITORY_NAME`), and any future authorized testimonials added to `assets/data/testimonials.csv`. The included illustrations and logo are local packaged assets. No telephone number, service area, pricing, or policies have been added because they were not supplied as supported source content.
